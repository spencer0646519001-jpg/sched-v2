...
_EXPLAIN_OUTCOME_TEMPLATES: dict[str, dict[str, str]] = {
    "zh": {
        "explain_ready": "已產生當日排班說明。",
        "explain_unsupported_request": "這裡只支援排班相關的當日說明請求。",
        "explain_out_of_context_date": "請求必須與所選日期和目前排班上下文一致。",
        "explain_unknown_worker": "無法在目前排班上下文中安全識別該員工。",
        "explain_candidate_required": "目前沒有可比較的候選預覽。",
        "explain_no_schedule_surface": "目前沒有可說明的排班結果。",
    },
    "ja": {
...
_EXPLAIN_RENDER_COPY: dict[str, dict[str, Any]] = {
    "zh": {
        "headline_day": "{target_date} 的排班說明",
        "headline_worker": "{target_date} 的 {worker_name} 排班說明",
        "sections": {
            "assignments": "排班摘要",
            "warnings": "警告摘要",
            "fallback": "回退摘要",
        ...
        "day_kinds": {"weekday": "工作日", "weekend": "週末"},
        "availability": {
            "leave": "當天已有請假紀錄。",
            "fixed_day_off": "當天屬於固定休息日。",
            "ad_hoc_unavailable": "當天被標記為臨時不可排班。",
            "wish_off_hard": "當天有強約束休假偏好。",
            "wish_off_soft": "當天有軟性休假偏好。",
        },
        "warning_labels": {
            "understaffed_station_day": "崗位人手不足",
            "missing_required_chef": "缺少必要廚師",
            "missing_morning_station_coverage": "早班崗位覆蓋不足",
            "worker_below_min_days_off": "員工休息天數不足",
            "duplicate_assignment_conflict": "重複排班衝突",
            "worker_unavailable_conflict": "員工不可排班衝突",
        },
        "notes": {
            "no_assignments": "當天沒有排班紀錄。",
            "no_preview_warnings": "當天沒有候選預覽警告。",
            "current_warning_gap": "目前工作區不會持久化日警告；這裡只能基於已顯示排班說明。",
            "no_fallback": "當天沒有回退排班。",
            "required_chef": "當天包含必要廚師覆蓋紀錄。",
            "no_change": "所選日期在候選預覽與目前工作區之間沒有排班變化。",
            "not_assigned": "{worker_name} ({worker_code}) 在這一天未被排班。",
            "assigned": "{worker_name} ({worker_code}) 已排班：{assignment}.",
            "no_blocking_reason": "目前上下文中沒有記錄到阻止該員工排班的硬性原因。",
            "skills_overlap": "{worker_name} 的崗位技能與當天崗位有重疊：{station_codes}。",
            "skills_no_overlap": "{worker_name} 的崗位技能與當天崗位沒有重疊。",
            "min_staff": "{day_kind} 最低人數目標：{min_staff_target}。",
            "stations": "崗位最低覆蓋：{stations}.",
            "max_staff": "單日最多排班人數：{max_staff_per_day}。",
            "max_consecutive_days": "最長連續排班天數限制：{max_consecutive_days}。",
            "min_rest_days_per_month": "每月最少休息天數：{min_rest_days_per_month}。",
            "require_one_chef": "該月啟用了每日至少一名廚師約束。",
            "fallback_assignment": "{worker_name} ({worker_code}) 以回退方式覆蓋 {station_code}。",
            "required_chef_assignment": "{worker_name} ({worker_code}) 負責必要廚師覆蓋。",
...
_WARNING_DETAIL_LABELS: dict[str, dict[str, str]] = {
    "zh": {
        ...
        "missing_staff": "缺口",
        "required_role": "必要角色",
        "minimum_days_off": "最少休息天數",
        "actual_days_off": "實際休息天數",
        "operation": "操作",
        "assignment_count": "排班數",
        "shift_code": "班次",
    },
    "ja": {
        "station_code": "持ち場",
